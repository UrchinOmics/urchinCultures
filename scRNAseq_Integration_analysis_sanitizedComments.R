...
## it also appends the percent mitochondrial reads to the metadata. 
...
# this looks bad. It is not. The explicit separation of the list into individual seurat objects and then mergining those objects by name is a necessary step. 
# merging with a loop - while vastly cleaner-looking here - will cause infuriating and nearly uninterpretable errors in the integration steps. 
# Seurat does not handle successive merge operations well at all. It should be avoided wherever possible, even if it means something that looks as silly as the following code. 
...
# Merge Seurat objects for cultures and atlas. This must be in a single operation. 
...
#save the raw seurat objects in case i need to backtrack. 
...
#filtering and joining (joining now is necessary to avoid weirdness in integration below)
...
#SCTtransform does not assume equal amounts of RNA per cell in normalization. 
...
LV_2_to_24hr_sct <- LV_2_to_24hr_sct %>% RunPCA() %>% RunUMAP(reduction = "pca", dims = 1:30)

LVcultures_sct <- SCTransform(LVcultures_filtered, method = "glmGamPoi", verbose = FALSE)
LVcultures_sct <- LVcultures_sct %>% RunPCA() %>% RunUMAP(reduction = "pca", dims = 1:30)
...
#integration
#trying RPCA here which is supposedly more efficient in finding anchors vs CCA (default, which hangs when ive tried it)
...
LVint <- FindNeighbors(LVint, dims = 1:50, verbose = FALSE) %>% FindClusters(resolution = 2, verbose = FALSE) #dims determined by elbow plot. resolution makes a similar number of clusters as we observe cell types in LV scRNA atlas
LVint <- RunPCA(LVint, verbose = FALSE)
LVint <- RunUMAP(LVint, reduction = "pca", dims = 1:30)
save(LVint, file = "LVint.Rda")
...
# Ensure `orig.ident` is a factor with correct order
...
DimPlot(LVint, reduction = "umap", group.by = "orig.ident", label = TRUE, )
ggsave(filename = "UMAP-sample.png", path = "IntegratedUMAPs")


#plot clusters
p3 <- DimPlot(LVint, reduction = "umap", group.by = "seurat_clusters")
p3
ggsave(filename = "umap-seurat-clusters.png", path = "IntegratedUMAPs")

#split by sample source and plot clusters - easier to see that there is indeed overlap given the big change in abundance from embryos to cultures. 
#fig 4D
...
#using this in supplement - modifying below for better visibility of sample source
...
plot_data <- plot_data %>%
  mutate(LOC121428528 = pmin(LOC121428528, 20)) #impose max cutoff to keep rare high count observations from blowing out color scaling. 


# Adjust Brewer palettes to start further up the scale for better visibility
greens_adjusted <- colorRampPalette(brewer.pal(9, "Greens")[3:9])(100) # Remove light greens
reds_adjusted <- colorRampPalette(brewer.pal(9, "Reds")[3:9])(100)    # Remove light reds

# Plot with adjusted gradients
ggplot(plot_data, aes(x = umap_1, y = umap_2)) +
  # Plot atlas with adjusted red-to-purple scale 
  geom_point(
  ...
  new_scale_color() +
  # Plot cultures with adjusted green-to-blue scale 
  geom_point(
...
ggsave(filename = "umap-pks1_v2.png", path = "IntegratedUMAPs/")

# LOC447794 - alx-1 
VlnPlot(LVint, features = "LOC447794")
ggsave(filename = "vln-alx1.png", path = "IntegratedUMAPs/")
FeaturePlot(object = LVint, features = "LOC447794", min.cutoff = 'q1', label = T)  + labs(title = "LOC447794 - alx1")#alx1
ggsave(filename = "umap-alx1.png", path = "IntegratedUMAPs/")

# LOC446151 - Brachyury
VlnPlot(LVint, features = "LOC446151")
ggsave(filename = "vln-brachyury.png", path = "IntegratedUMAPs/")
FeaturePlot(object = LVint, features = "LOC446151", min.cutoff = 'q1', label = T) + labs(title = "LOC446151 - brachyuri") #Brachyury
ggsave(filename = "umap-brachyury.png", path = "IntegratedUMAPs/")

#Seawi LOC121431753
VlnPlot(LVint, features = "LOC121431753")
ggsave(filename = "vln-seawi.png", path = "IntegratedUMAPs/")
FeaturePlot(object = LVint, features = "LOC121431753", label = T) + labs(title = "LOC121431753 - seawi") #seawi
ggsave(filename = "umap-seawi.png", path = "IntegratedUMAPs/")

#Vasa LOC121424246
VlnPlot(LVint, features = "LOC121424246")
FeaturePlot(object = LVint, features = "LOC121424246", label = T) + labs(title = "LOC121424246 - vasa")#vasa

#Nanos2 LOC121408828
VlnPlot(LVint, features = "LOC121408828")
ggsave(filename = "vln-nanos2.png", path = "IntegratedUMAPs/")
FeaturePlot(object = LVint, features = "LOC121408828", label = T) + labs(title = "LOC121408828 - nanos") #nanos
ggsave(filename = "umap-nanos2.png", path = "IntegratedUMAPs/")

#tcf - LOC121432141
VlnPlot(LVint, features = "LOC121431848")
ggsave(filename = "vln-tcf.png", path = "IntegratedUMAPs/")
FeaturePlot(object = LVint, features = "LOC121431848", label = T) + labs(title = "LOC121431848 - tcf")#tcf
...
# Fetch UMAP coordinates, gene expression, and metadata for the specified gene
...
plot_data <- plot_data %>%
  mutate(LOC121406173 = pmin(LOC121406173, 10)) #impose max count cutoff per cell of 10 (above 10 just registers as the maximum). this prevents a few very strongly expressing cells from blowing out the scaling. 
...
ggsave(filename = "umap-endo16_v2.png", path = "IntegratedUMAPs/")

#foxa1 LOC121419318
VlnPlot(LVint, features = "LOC121419318")
ggsave(filename = "vln-foxa1.png", path = "IntegratedUMAPs/")
FeaturePlot(object = LVint, features = "LOC121419318", label = T) + labs(title = "LOC121419318 - foxa1")
ggsave(filename = "umap-foxa1.png", path = "IntegratedUMAPs/")


# tektin-1-like LOC121408954
VlnPlot(LVint, features = "LOC121408954")
ggsave(filename = "vln-tektin-1-like.png", path = "IntegratedUMAPs/")
FeaturePlot(object = LVint, features = "LOC121408954", min.cutoff = 'q1', label = T) + labs(title = "LOC121408954 - tektin-1-like")
ggsave(filename = "umap-tektin-1-like.png", path = "IntegratedUMAPs/")

# cilia-_and_flagella-associated_protein_251-like LOC121411688
VlnPlot(LVint, features = "LOC121411688")
ggsave(filename = "vln-cilia-_and_flagella-associated_protein_251-like.png", path = "IntegratedUMAPs/")
FeaturePlot(object = LVint, features = "LOC121411688",  label = T) + labs(title = "LOC121411688 - cilia-_and_flagella-associated_protein_251-like")
...
VlnPlot(LVint, features = "LOC121405827")
ggsave(filename = "vln-G1.S-specific_cyclin-D1-like.png", path = "IntegratedUMAPs/")
FeaturePlot(object = LVint, features = "LOC121405827", min.cutoff = 'q1', label = T) + labs(title = "LOC121405827	- G1/S-specific_cyclin-D1-like")
...
VlnPlot(LVint, features = "LOC121417436")
ggsave(filename = "vln-proliferation-associated_protein_2G4-like.png", path = "IntegratedUMAPs/")
FeaturePlot(object = LVint, features = "LOC121417436", min.cutoff = 'q1', label = T) + labs(title = "LOC121417436 - proliferation-associated_protein_2G4-like")
...
VlnPlot(LVint, features = "LOC121428657")
ggsave(filename = "vln-hnf4-foxa1.png", path = "IntegratedUMAPs/")
FeaturePlot(object = LVint, features = "LOC121428657", min.cutoff = 'q1', label = T) + labs(title = "LOC121428657	-	hepatocyte_nuclear_factor_4-alpha-like (FOXA1)")
...
FeaturePlot(object = LVint, features = "LOC121419318", min.cutoff = 'q1', label = T) + labs(title = "LOC121419318	-	hepatocyte_nuclear_factor_3-beta-like (FOXA2)")
ggsave(filename = "umap-hnf3bl-foxa2.png", path = "IntegratedUMAPs/")

# LOC121430133 -	transcription_factor_Sox-2-like
VlnPlot(LVint, features = "LOC121430133")
ggsave(filename = "vln-sox2-like.png", path = "IntegratedUMAPs/")
FeaturePlot(object = LVint, features = "LOC121430133", min.cutoff = 'q1', label = T) + labs(title = "LOC121430133 -	transcription_factor_Sox-2-like")
ggsave(filename = "umap-sox2-like.png", path = "IntegratedUMAPs/")

# LOC121418887	-	homeobox_protein_six1-like
VlnPlot(LVint, features = "LOC121418887")
ggsave(filename = "vln-homeobox_protein_six1-like.png", path = "IntegratedUMAPs/")
FeaturePlot(object = LVint, features = "LOC121418887", min.cutoff = 'q1', label = T) + labs(title = "LOC121418887 	-	homeobox_protein_six1-like")
ggsave(filename = "vln-homeobox_protein_six1-like.png", path = "IntegratedUMAPs/")
...
filtered_metatable <- metatable %>%
  group_by(cluster) %>%
  filter(all(counts[sample_source == "atlas"] >= 5, counts[sample_source == "cultures"] >= 5))

library(forcats) # For fct_reorder

# Order clusters by counts in cultures
...
ggsave("cellclustdiff.png", width = 3, height = 4, units = "in") #save - plot will get cropped but this makes correct size for figure. 


###### VENN DIAGRAM FIG 4B ######
#this is not scaled. venn diagrams are awful but here I think it gets the point across. 
